			...
			<div class="col-xs-12">
				<div class="page-header" id="report_title_container">
					<div class="row">
						<div class="col-xs-2">
							<select class="form-control input-sm" id="report_type">
								...
								<optgroup label="선거">
									...
									<option value="1" selected>선거속보</option>
									@else
									<option value="1">선거속보</option>
									@endif

									@if($report->report_type == 2)
									<option value="2" selected>일일보고</option>
									@else
									<option value="2">일일보고</option>
									@endif
								</optgroup>
								<optgroup label="행사">
									@if($report->report_type == 3)
									<option value="3" selected>행사속보</option>
									@else
									<option value="3">행사속보</option>
									@endif
								</optgroup>
								@else
								<optgroup label="선거">
						...
						<div class="col-xs-10">
							<input type="text" id="report_title" class="form-control col-xs-12 input-sm" value="{{ $report->title or '' }}" placeholder="제목을 입력하세요">
						</div>
					</div>
					<br>
					@if ($mode == 'edit')
...
$(function() {
	...
	$("#report_submit").click(function() {
		...
		var reportType = $("#report_type option:selected").val();
		...
		switch (mode) {
			...
			break;
			case 'create':
				url = base_url+"/eventreports/create";
			break;
			default:
			return;
		}
	...
	$("#report_save_draft").click(function() {
		...
		$.ajax({
			url: base_url+"/eventreports/drafts/save",
			type: "post",
			data: params,
			success: function(res) {
				alert(res.message);
	...
	$("#select_report_template_btn").click(function() {
		popup(base_url+"/eventreports/templates", 300, 400);
	});

	$("#select_report_drafts_btn").click(function() {
		popup(base_url+"/eventreports/drafts", 600, 800);
	});
	...
	$("#report_create_template").click(function() {
		...
		$.ajax({
			url: base_url+"/eventreports/templates/save",
			type: "post",
			data: params,
			success: function(res) {
				alert(res.message);
...
function onTemplateSelected(template, id) {
	vHwpCtrl.SetTextFile(template.content, "HWP", "");
	$("#report_type option:selected").attr("selected", false);
	$("#report_type").val(id).attr('selected',true);
}
