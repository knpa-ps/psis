		...
		<form id="insert_form" class="form-horizontal" novalidate>
			...
			<fieldset>
				...
				<div class="row">
					...
					<div class="form-group col-xs-4">
						...
						<div class="col-xs-7">
							{{ View::make('widget.code-select', array('id'=>'sit_code', 
							'category'=>'B002', 
							'blank'=>false, 
			...
			<fieldset>
				...
				<table id="detail_table" class="table table-condensed table-bordered table-hover table-striped">
					...
					<tbody>
						@foreach ($master->details as $idx=>$detail)	
						<tr class="data-row">
							<td>
								<input type="hidden" class="data-detail-id" value="{{ $detail->id }}">
								<div class="dept-selector" id="test">
									<input type="text" class="input-sm data-dept-name col-xs-12" readonly="readonly"
									name="dept_names[{{$idx+1}}]"
									value="{{ $detail->department->full_name }}">
									<input type="hidden" name="dept_ids[{{$idx+1}}]" class="data-dept-id"
							...
							<td>
								<input type="text" class="input-sm col-xs-12 data-name" name="names[{{ $idx+1 }}]"
								value="{{ $detail->name }}">
							</td>
							<td>
								<div class="col-xs-3 ">
									<input type="text" class="input-sm col-xs-12 data-start-date input-datepicker" name="start_dates[{{ $idx+1 }}]" placeholder="Y-m-d"
									value="{{ date('Y-m-d', strtotime($detail->start)) }}">
								</div>
								<div class="col-xs-2">
									<input type="text" class="input-sm col-xs-12 data-start-time" name="start_times[{{$idx+1}}]" placeholder="00:00"
									value="{{ date('H:i', strtotime($detail->start)) }}">
								</div>
								<div class="col-xs-1" align="center">~</div>
								<div class="col-xs-3">
									<input type="text" class="input-sm col-xs-12 data-end-date input-datepicker" name="end_dates[{{$idx+1}}]" placeholder="Y-m-d"
									value="{{ date('Y-m-d', strtotime($detail->end)) }}">
								</div>
								<div class="col-xs-2">
									<input type="text" class="input-sm col-xs-12 data-end-time" name="end_times[{{$idx+1}}]" placeholder="00:00"
									value="{{ date('H:i', strtotime($detail->end)) }}">
								</div>
							</td>
							<td>
								<input type="text" class="input-sm col-xs-12 data-amount" name="amounts[{{ $idx+1 }}]"
								value="{{ $detail->amount }}">
							</td>
							<td>
...
<table class="hide" id="form_template_table">
	<tbody>
		<tr class="data-row">
			<td>
				<input type="hidden" class="data-detail-id">
				<div class="dept-selector" id="test">
					<input type="text" class="input-sm data-dept-name col-xs-12" readonly="readonly">
	...
	var validator = $("#insert_form").validate({
		...
		submitHandler: function(form) {
			...
			$("#detail_table .data-row").each(function() {
				var detail = {
					id : $(this).find('.data-detail-id').val(),
