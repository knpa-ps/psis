...
@include('budget.config.nav')

<div class="row-fluid">		
	<div class="panel panel-default span6">
		<div class="panel-heading well" data-original-title>
			<h3>경비동원수당 입력마감일</h3>
		</div>
		<div class="panel-body">
			<form id="budget_mob_form" class="form-inline form well well-small">
				<label for="budget.mob.close_date" class="label label-inline">기본 마감일</label>
				<input type="text" class="input-mini" value="{{ $configs['budget.mob.close_date'] }}" name="budget.mob.close_date">
				<label for="budget.mob.close_time" class="label label-inline"  >기본 마감시간</label>
				<input type="text" class="input-mini" value="{{ $configs['budget.mob.close_time'] }}" name="budget.mob.close_time">
				...
				<table class="table table-condensed table-hover table-bordered table-striped">
					...
					<tbody>
						...
						<tr id="{{ $cost->id }}">
							...
							<td>
								<input type="text" name="cost_{{$cost->id}}" value="{{$cost->cost}}" class="input-mini">
							</td>
						</tr>
						@endforeach
					</tbody>
				</table>
				<button type="button" class="btn btn-primary" id="cost_form_submit">저장</button>
			</form>
		</div>
	</div>
</div>
...
$(function(){
	...
	$("#cost_form_submit").click(function(){
		var params = $("#cost_form").serializeArray();

		$.ajax({
			url: "{{ action('BgConfigController@updateMobCost') }}?type=mob",
			type: "post",
			data: params,
			success: function(resp) {
				...
				{
					noty({type:'error', layout: 'topRight', text: '오류가 발생했습니다.'});	
				}
			}
		});
	});
	...
	$("#create_close_date_form").submit(function(){
		...
		$.ajax({
			url: "{{ action('BgConfigController@createCloseDate') }}?type=mob",
			type: "post",
			data: params,
	...
	$("#delete-selected").click(function(){
		...
		$.ajax({
			url: "{{ action('BgConfigController@deleteCloseDates') }}?type=mob",
	...
	$("#budget_mob_form").submit(function(){
		var params = $(this).serializeArray();
		$.ajax({
	...
	var oTable = $("#close_date_table").dataTable($.extend(dtOptions,{
		...
		"bProcessing": true,
		"bServerSide": true,
		"sAjaxSource": "{{ action('BgConfigController@readCloseDates') }}?type=mob"
	}));
